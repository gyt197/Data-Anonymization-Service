...
(function (S1) {
    ...
    /**
     * Downloads synthetic CSV as a local file via Blob immediately.
     * Avoids opening CSV text in a new browser tab.
     * @param {string} selectedTableName - Selected table name
     * @param {string} baseUrl
     * @returns {Promise<void>}
     */
    function downloadSyntheticCsv(selectedTableName, baseUrl) {
    ...
    function refreshStep4SourceDropdown() {
      ...
      if (supported.length) {
        var tableNameFromFilename = (window.Step1 && window.Step1.tableNameFromFilename)
          ? window.Step1.tableNameFromFilename
          : function (name) { return (name || '').replace(/\.(csv|xls|xlsx)$/i, ''); };
    ...
    function initStep4(opts) {
            ...
            if (configForTable) {
              ...
              if (tbody) {
                tbody.querySelectorAll('tr').forEach(function (tr) {
                  ...
                  // Support both radio buttons (new) and checkboxes (legacy fallback)
                  var forceInput = tr.querySelector('.force-pii-radio') || tr.querySelector('.force-pii-chk') || tr.querySelector('td:nth-child(3) input');
                  var exemptInput = tr.querySelector('.exempt-radio') || tr.querySelector('.exempt-chk') || tr.querySelector('td:nth-child(4) input');
  
                  if (exemptInput && exemptInput.checked) exempt.push(colName);
                  if (forceInput && forceInput.checked) forcePii.push(colName);
                });
              }
            } else if (onToast) {
      ...
      // FIX: Download button now triggers the file download directly on first click.
      // Previously it called renderInlineDownload() which only rendered a *second* button
      // inside the content area — requiring two clicks to actually download.
      // Now it calls downloadSyntheticCsv() immediately, with loading feedback on the
      // button itself and a toast on success/failure.
      if (downloadBtn) {
        var downloadBtnDefaultHTML = downloadBtn.innerHTML;
        ...
        downloadBtn.addEventListener('click', function () {
          ...
          downloadSyntheticCsv(t, baseUrl)
            .then(function () {
              if (onToast) onToast('CSV downloaded: synthetic_' + t + '.csv', 'success');
            })
            .catch(function (err) {
            ...
            .finally(function () {
              ...
              downloadBtn.innerHTML = downloadBtnDefaultHTML;
            });
        });
      }
      ...
      var deleteAllBtn = document.getElementById('btn-step4-delete-all');
      if (deleteAllBtn) {
        deleteAllBtn.addEventListener('click', function () {
          var confirmed = window.confirm(
            'Delete ALL uploaded raw data and generated results?\n\nThis cannot be undone.'
          );
          if (!confirmed) return;
  
          deleteAllBtn.disabled = true;
          var originalLabel = deleteAllBtn.textContent;
          deleteAllBtn.textContent = 'Deleting…';
  
          fetch((baseUrl || API_BASE).replace(/\/$/, '') + '/api/delete-all', { method: 'DELETE' })
            .then(function (res) {
            ...
            .then(function (body) {
              /* Reset the table dropdown — nothing left to show */
              refreshTableDropdown([], {}, baseUrl);
              var area = document.getElementById('step4-content-area');
              if (area) area.innerHTML = '';
              setStep4Error('');
              if (onToast) onToast(body.message || 'All data deleted.', 'success');
            })
            .catch(function (err) {
              if (onToast) onToast('Delete failed: ' + (err.message || String(err)), 'error');
            })
            .finally(function () {
